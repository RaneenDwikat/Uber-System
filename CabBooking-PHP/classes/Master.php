...
Class Master extends DBConnection {
    ...
    function delete_cab(){
        extract($_POST);
        $del = $this->conn->query("UPDATE `cabs` set `delete_flag` = 1  where id = '{$id}'");
        if($del){
            $resp['status'] = 'success';
    ...
    function save_booking(){
        ...
        $date0=date("d/m/Y");
        $time0=date("h:i A");
        $this->conn->query("INSERT INTO `federal` (`Date_`,`Time_`,`PU_Add`,`DO_Address`,`Status`) values
        ('$date0','$time0','$pickup_zone','$drop_zone','$status')");

        if(empty($id)){
            $sql = "INSERT INTO `booking_list` (`client_id`,`cab_id`,`pickup_zone`,`drop_zone`,`status`) values
        ('$cid','$cab_id','$pickup_zone','$drop_zone','$status') ";
            $save = $this->conn->query($sql);
        }else{
