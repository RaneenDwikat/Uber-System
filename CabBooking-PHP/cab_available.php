        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="row gx-4 gx-lg-5 row-cols-1 row-cols-sm-1 row-cols-md-2 row-cols-xl-3" id="cab_list">
                    <?php
                    $cabs = $conn->query("SELECT * FROM cabs WHERE active ='Yes'");
                    while($row= $cabs->fetch_assoc()):
                        ?>
                        <a class="col item text-decoration-none text-dark book_cab" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>" data-bodyno="<?php echo $row['DMV_License_Plate_Number'] ?>">
                            <div class="callout callout-primary border-success rounded-0">
                                <dl>
                                    <dt class="h3"><i class="fa fa-taxi"></i> <?php echo $row['DMV_License_Plate_Number'] ?></dt>
                                    <dd class="truncate-3 text-muted lh-1">
                                        <small><?php echo $row['Vehicle_Year'] ?></small><br>
                                        <small><?php echo $row['Name'] ?></small><br>
                                        <small><?php echo $row['Base_Telephone_Number'] ?></small>
